    ...
    def __init__(self,
        ...
        for i in range(len(n_filter)):
            if apply_weight_reg[i]:
                ...
                a = tf.Variable(tf.multiply(1.0, tf.ones([1])), name='a_w%d' % i)
                b = tf.Variable(tf.add(0.0, tf.zeros([1])), name='b_w%d' % i)
                w1 = tf.add(tf.multiply(a, w1, name='mul%d' % i), b, name='add%d' % i)
                loss = tf.nn.l2_loss(tf.subtract(w1, w2, name='sub_conv%d' % i), name='l2_conv%d' % i)
                self.objective = tf.add(self.objective, tf.multiply(self.weight_lambda, loss, name='mul_w%d' % i), name='add_w%d' % i)
        ...
        for i in range(len(n_filter), len(n_filter) + len(n_hidden)):
            if apply_weight_reg[i]:
                ...
                b = tf.Variable(tf.add(0.0, tf.zeros([1])), name='b_w%d' % i)
                w1 = tf.add(tf.multiply(a, w1, name='mul%d' % i), b, name='add%d' % i)
                loss = tf.nn.l2_loss(tf.subtract(w1, w2, name='sub_fc%d' % i), name='l2_fc%d' % i)
                self.objective = tf.add(self.objective, tf.multiply(self.weight_lambda, loss, name='mul_w%d' % i), name='add_w%d' % i)
